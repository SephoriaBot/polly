...
// This is the fix for the "two hamsters from one egg" bug: CreatureBreeder
// (formerly CreatureNest) and CreatureHabitat used to each call the growth
// hook independently, which meant two separate instances both raced to
// check + award the same real-world accomplishment before either had
// updated hamster_last_check — double-counting it and sometimes
// double-hatching/double-evolving as a result.
//
// Wrap the part of the app that renders CreatureBreeder/CreatureHabitat in
...
export function useCreatureGrowth() {
  ...
  if (!ctx) {
    throw new Error("useCreatureGrowth must be used within a <CreatureGrowthProvider>. Wrap the section of Dashboard.tsx that renders CreatureBreeder/CreatureHabitat.");
  }
  return ctx;
}