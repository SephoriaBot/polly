...
export type EnergyMode = 'normal' | 'noEnergy';
...
// No Energy Mode is meant to be a "today only" state, not a permanent
// setting someone forgets they turned on — so the stored value is scoped
// to a date. A new day always starts back at Normal.
export function EnergyProvider({ children }: { children: ReactNode }) {
  const [mode, setModeState] = useState<EnergyMode>(() => {
    try {
      const stored = JSON.parse(localStorage.getItem(STORAGE_KEY) || 'null');
      if (stored && stored.date === todayISO() && stored.mode === 'noEnergy') {
        return 'noEnergy';
      }
  ...
  // date check rather than silently staying in a stale no-energy state.
  useEffect(() => {
