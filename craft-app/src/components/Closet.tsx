...
export default function Closet() {
  ...
  async function handleEquip(cosmeticId: string, alreadyEquipped: boolean) {
    setPendingId(cosmeticId);
    await setEquippedHeadwear(alreadyEquipped ? null : cosmeticId);
    setPendingId(null);
  }
      ...
      >
        {owned.map(item => {
          ...
          return (
            <button
              key={item.cosmeticId}
              onClick={() => handleEquip(item.cosmeticId, isEquipped)}
              disabled={isPending}
              style={{
                display: 'flex',
