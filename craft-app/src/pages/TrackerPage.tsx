...
type TabType = TrackerType | 'habits' | 'overlap';
...
export default function TrackerPage({
  ...
  const [cycleMoon, setCycleMoon] = useState<{
    start: MoonPhase | null;
    end: MoonPhase | null;
  }>({
    start: null,
    end: null,
  ...
  async function handleAddTracker() {
    ...
    try {
      const def = await addCustomTracker(
        label,
        newTrackerUnit.trim()
      );
  ...
  async function handleRemoveTracker(def: CustomTrackerDef) {
    if (
      !window.confirm(
        `Remove "${def.label}" and all its logged entries? This can't be undone.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setCustomTrackers((prev) =>
        prev.filter((t) => t.id !== def.id)
      );

      if (activeType === def.id) {
        setActiveType('habits');
      }
    } catch (e) {
      console.error('failed to remove custom tracker', e);
    }
  }
    ...
    getTrackerLogsInRange(
      ...
      .then((logs) => {
        const starts = logs
          .filter(
            (l) =>
              (l.value as PeriodValue).bleeding_start
          )
          .map((l) => l.log_date)
          .sort()
          .reverse();
        ...
        const endLog = logs
          .filter(
            (l) =>
              (l.value as PeriodValue).bleeding_end &&
              l.log_date >= lastStartDate
          )
          .map((l) => l.log_date)
          .sort()[0];
        ...
        setCycleMoon({
          start: getMoonPhase(
            new Date(`${lastStartDate}T12:00:00Z`)
          ),
          end: endLog
            ? getMoonPhase(
                new Date(`${endLog}T12:00:00Z`)
              )
            : null,
        });
      })
      .catch((e) => {
        console.error('failed to load period information', e);
      });
  }, [refreshKey]);
  ...
  const activeCustomTracker = customTrackers.find(
    (t) => t.id === activeType
  );

  const isHabits = activeType === 'habits';
  const isOverlap = activeType === 'overlap';
    ...
    <div>
      ...
      <div className="page-header">
        <div className="title-row">
          <h2>
            <Icon name="title-tracker" size={22} />
            <PageTitleLogo name="tracker" height={28} />
          </h2>
        </div>
      </div>

      {/* HABIT TRACKER GETS ITS OWN PAGE/TAB */}
      {isHabits ? (
        <>
          <div
            style={{
          ...
          >
            <button
              ...
              onClick={() => setActiveType('habits')}
            >
              <Icon name="icon-check" size={16} />
              Habits
            </button>

            {(Object.keys(TRACKER_CONFIG) as TrackerType[]).map(
              (type) => (
                <button
                  key={type}
                ...
                >
                  {TRACKER_CONFIG[type].icon ? (
                    <Icon
                      name={TRACKER_CONFIG[type].icon!}
                      size={16}
                    />
                  ) : (
                    TRACKER_CONFIG[type].emoji
                  )}{' '}
                  {TRACKER_CONFIG[type].label}
                </button>
              )
            )}
        ...
        <>
          ...
          {cycleDay !== null && (
            ...
            >
              <div>
                <Icon name="flower" size={16} /> Day {cycleDay} of
                your cycle
                  ...
                  {cycleMoon.start && (
                    ...
                    >
                      <Icon
                        name={
                          (MOON_ICON_BY_PHASE[
                            cycleMoon.start.phaseName
                          ] ?? 'moon-cloud') as IconName
                        }
                        size={14}
                      />
                      ...
                      Started under{' '}
                      {cycleMoon.start.phaseName} (
                      {cycleMoon.start.illuminationPct}%)
                    </span>
                  )}
                  ...
                  {cycleMoon.end && (
                    ...
                    >
                      <Icon
                        name={
                          (MOON_ICON_BY_PHASE[
                            cycleMoon.end.phaseName
                          ] ?? 'moon-cloud') as IconName
                        }
                        size={14}
                      />

                      Ended under {cycleMoon.end.phaseName} (
                      {cycleMoon.end.illuminationPct}%)
                    </span>
                  )}
                </div>
              )}
          ...
          {showAddTracker && (
            ...
            >
              ...
              >
                Track anything with a single daily number — water
                intake, pages read, mood score, whatever's useful
                to you.
              </div>
              ...
              >
                <input
                  ...
                  type="text"
                  placeholder="Name (e.g. Water Intake)"
                  value={newTrackerLabel}
                  onChange={(e) =>
                    setNewTrackerLabel(e.target.value)
                  }
                  style={{ width: 200 }}
                />
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewTrackerUnit(e.target.value)
                  }
                  style={{ width: 140 }}
                />
          ...
          {activeCustomTracker && (
            ...
            >
              <button
                className="btn-secondary"
                onClick={() =>
                  handleRemoveTracker(activeCustomTracker)
                }
              >
                <Icon name="icon-trash2" size={24} />
                Remove "{activeCustomTracker.label}"
              </button>
            </div>
          )}
